...
package com.tcc.demo.springcloud.order.controller;
...
public class OrderController {
	...
	@PostMapping(value = "/orderNestedPay")
	@ApiOperation(value = "订单支付接口（注意这里模拟的是创建订单并进行支付扣减库存等操作）")
	public String orderNestedPay(OrderDTO orderDto) {
		return orderService.orderNestedPay(orderDto);
	}

	@PostMapping(value = "/orderNestedPayException")
	@ApiOperation(value = "订单支付接口（注意这里模拟的是创建订单并进行支付扣减库存等操作）")
	public String orderNestedPayException(OrderDTO orderDto) {
		return orderService.orderNestedPayException(orderDto);
	}
	...
	public String mockInventoryWithTryException(OrderDTO orderDto) {
		String result = "";
		try {
			result = orderService.mockInventoryWithTryException(orderDto);
		} catch (Exception e) {
	...
	public String mockInventoryWithTryTimeout(OrderDTO orderDto) {
		return orderService.mockInventoryWithTryTimeout(orderDto);
	}
